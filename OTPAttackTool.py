...
def jam(nomor):
    ...
    for _ in range(10):
        try:
            ...
            if RTO_flag ==0:
                print("")
                typing("--Request Time Out--")
                print("test")
                
        except requests.exceptions.ConnectionError:
            print("")
            typing("--Fail To Establish a New Connection--")
            time.sleep(1000)
            rto = 1
            ...
            print("")
            typing("--A Connection Attemp Failed Because The Connect Party Did Not Properly Respond After a Period of time--")
            time.sleep(1000)
            rto = 1
            
        except urllib3.exceptions.ProtocolError:
            print("")
            typing("--A Connection Attempt Failed Because The connected Party Did Not Proprly Respond After A Period Of TIme--")
            time.sleep(1000)
            rto = 1
...
def start(nomor,x):
    if x == 0:
        os.system("cls")
        typing(F"{red}Infinity Loop Spam To {white} {nomor} is {green} Ready")
        jam(nomor)
    else:
        print("")
        typing("--reboot wait 20 Seccond--")
        time.sleep(15)
        os.system("cls")
        typing(F"{red} Looping For Number : {nomor} .... {green}")
        jam(nomor)
        

def main():
    typing(f"Selamat datang di {red}MySpamBot")
    print(f"""{yellow}Author      : {green}Ricky Khairul Faza
{yellow}Github      : {red}github.com/rickyfazaa
{yellow}Instagram   : {blue}instagram.com/rickyfazaa""")
    # Contoh : 089508226367
    print(nomor := input(f"{green}Masukkan Nomor Target: {white}")) # Walrus Operator untuk inputan Nomor Target
